...
errormap = ["", "마음", "트윗", "", "트윗중 사진"]
titlemap = {'likes' : '이 누른 마음', 'tweets' : "의 트윗"}
typemap = {'likes' : [0b000, 0b001], 'tweets' : [0b010, 0b110]}
...
class Data():
    ...
    def add_type(self, channel_id, id, newtype):
        ...
        if before & newtype:
            raise Exception(f"이미 채널에서 해당 계정의 {errormap[newtype]}을 불러오고 있습니다.")
        if newtype == 0b001:
...
class Main():
    ...
    def update_history(self, id, st):#예외 넘김 - 보호몰라, 로딩 실패
        def append(value, state):
            ...
            self.data.add_print(id, {'type' : typemap[st][1], 'title' : f'{self.data.get_user_name(id=id)} 님{titlemap[st]}', 'des': des, 'pic_url': value['media_url'], 'url': value['display_url']})
        ...
        if not self.data.get_user_protected(id):
            for state in Twitter(id=id).get_tweets(st):
                if self.data.add_history(id, state['id_str'], st+"_history"):
                    self.data.add_print(id, {'type' : typemap[st][0], 'title' : f'{self.data.get_user_name(id=id)} 님{titlemap[st]}', 'des': state['text'], 'pic_url': '', 'url': ''})
